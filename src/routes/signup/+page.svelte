<script lang="ts">
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		try {
			...
			if (result.success) {
				successMessage = 'Account created successfully! Check your inbox for a verification link.';
				setTimeout(() => {
					goto(resolve('/'));
				}, 1800);
			} else {
				errorMessage = result.error || 'Failed to create account. Please try again.';
			}
		} catch (err) {
...
>
	<div class="w-full max-w-md space-y-8">
		...
		<div class="text-center">
			...
			<h1 class="mt-6 text-2xl font-bold tracking-tight text-white sm:text-3xl">
				Create your account
			</h1>
			<p class="mt-2 text-sm text-slate-400">
				Master Korean vocabulary with smart spaced repetition
			</p>
		</div>
		...
		>
			...
			{#if errorMessage}
				...
				>
					...
					<div class="text-sm">
						<p class="font-medium text-rose-200">Registration error</p>
			...
			{#if successMessage}
				...
				>
					...
					<div class="text-sm">
						<p class="font-medium text-emerald-200">Welcome aboard!</p>
			...
			<form onsubmit={handleSubmit} class="space-y-4">
				<!-- Full Name Field -->
				<div>
					<label for="signup-name" class="block text-sm font-medium text-slate-200">
						Full Name
					</label>
				...
				<!-- Email Field -->
				<div>
					<label for="signup-email" class="block text-sm font-medium text-slate-200">
						Email address
					</label>
					<div class="relative mt-1.5">
						<div
				...
				<div>
					<div class="flex items-center justify-between">
						<label for="signup-password" class="block text-sm font-medium text-slate-200">
							Password
						</label>
						{#if password.length > 0}
							<span class="text-xs {passwordValid ? 'text-emerald-400' : 'text-amber-400'}">
				...
				<div>
					<div class="flex items-center justify-between">
						<label for="signup-confirm-password" class="block text-sm font-medium text-slate-200">
							Confirm Password
						</label>
						{#if confirmPassword.length > 0}
							<span class="text-xs {passwordsMatch ? 'text-emerald-400' : 'text-rose-400'}">
				...
				<div class="pt-1">
					<label class="flex items-start gap-2.5 text-xs text-slate-400">
						<input
							...
							required
							class="mt-0.5 h-4 w-4 rounded-md border-slate-700 bg-slate-800 text-indigo-600 focus:ring-indigo-500 focus:ring-offset-slate-900"
						/>
						<span>
							I agree to the <a href="#terms" class="text-indigo-400 hover:underline"
								>Terms of Service</a
							>
